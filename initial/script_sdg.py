...
#   lr = lr_initial * np.exp(-np.log(2)/1500*i)
	...
	if (i % 25 == 0):
		...
		if (maxmax <= testaccuracy[(i//25)]):
			...
			np.savez("sdg/max_model3.npz", **model)
			np.save("sdg/min_loss",loss)
...
np.save("sdg/test_loss_curve", loss_test)
np.save("sdg/train_loss_curve", loss)
np.save("sdg/batches_accuracys", batch_accuracy)
np.save("sdg/test_accuracy", testaccuracy)
