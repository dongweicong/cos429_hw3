...
#   lr = lr_initial * np.exp(-np.log(2)/1500*i)
	...
	if (i % 25 == 0):
		...
		if (maxmax <= testaccuracy[(i//25)]):
			...
			np.savez("sdg01/max_model2.npz", **model)
			np.save("sgd01/min_loss",loss)
...
np.savez("sdg01/final_model", **model)
np.save("sdg01/loss_curve", loss)
np.save("sdg01/batches_accuracys", batch_accuracy)
np.save("sdg01/test_accuracy", testaccuracy)
