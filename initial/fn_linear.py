...
def fn_linear(input, params, hyper_params, backprop, dv_output=None):
    ...
    if backprop:
        ...
        dv_input = np.matmul(W.T, dv_output)
        grad['W'] = np.matmul(dv_output, input.T) / batch_size
        grad['b'] = dv_output.sum(axis = 1) / batch_size
        grad['b'] = np.reshape(grad['b'],(num_out, 1))
